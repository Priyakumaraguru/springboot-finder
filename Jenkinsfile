pipeline {
 ...
 tools{
    ...
    stages {
     ...
     sh 'sshpass -p ${ansadmin_password} ssh -v -o StrictHostKeyChecking=no ansadmin@172.31.36.158 \"cd /etc/ansible/playbooks/target; wget -O sfinder-0.0.1-SNAPSHOT.war http://3.128.188.221:8080/var/lib/jenkins/workspace/springboot-finder/target/sfinder-0.0.1-SNAPSHOT.war \"' 
}
}
        }
   stage('deploy')
